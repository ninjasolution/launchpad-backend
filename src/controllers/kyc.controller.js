  ...
  response = await axios(createApplicant(externalUserId, levelName))
    .then(function (response) {
      ...
      .exec(async (err, user) => {
        ...
        user.applicantId = response.data.id;
...
exports.addDocument = (req, res) => {
  axios(addDocument(req.body.applicantId))
  .then(function (response) {
    console.log("Response:\n", response.data);
    return res.status(200).send({
      message: config.RES_MSG_SAVE_SUCCESS,
...
exports.getStatus = (req, res) => {
  axios(getApplicantStatus(req.body.applicantId))
  .then(function (response) {
    console.log("Response:\n", response.data);
    return res.status(200).send({
      message: config.RES_MSG_SAVE_SUCCESS,
...
exports.getTokens = (req, res) => {
  axios(createAccessToken(req.body.externalUserId, req.body.levelName, 1200))
  .then(function (response) {
    console.log("Response:\n", response.data);
    return res.status(200).send({
      message: config.RES_MSG_SAVE_SUCCESS,
